...
func makeboneboxmesh(skel, iboneparent, ibone):
	...
	var t0 = skel.get_bone_global_rest(iboneparent)
	var tp = skel.get_bone_rest(ibone)
	var t1 = skel.get_bone_global_rest(ibone) # = t0*tp
	...
	boxmesh.size = Vector3(0.015, boneleng, 0.018)
...
func _ready():
	var mesh : ArrayMesh = $BoxSkel.mesh
